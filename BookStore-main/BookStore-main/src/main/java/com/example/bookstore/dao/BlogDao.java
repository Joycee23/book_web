...
//
//	@Query("SELECT c FROM Blog c WHERE c.Deleteday IS NULL AND c.active = true")
...
public interface BlogDao extends JpaRepository<Blog, Integer> {
	@Query("SELECT c FROM Blog c WHERE c.Deleteday IS NULL")
	List<Blog> getListBlog();

	@Query("SELECT c FROM Blog c WHERE c.Deleteday IS NULL AND c.active = 1")
	Page<Blog> findAllBlogActive(Pageable pageable);
